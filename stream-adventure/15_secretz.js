...
tarParser.on('entry', function (e) {
    if (e.type == "File") {
        ...
        e.pipe(md5HashStream).pipe(concatStream(function(data){
            process.stdout.write(data + " " + e.path + "\n");
        }));
    }
});

var decryptStream = crypto.createDecipher(cipherName, passphrase);
var gunzipStream = zlib.createGunzip();

process.stdin.pipe(decryptStream).pipe(gunzipStream).pipe(tarParser);